/**
 * Created by yhf on 2020/11/1.
 * 参照workbench。但需要删除不少不需要的数据获取的逻辑
 */
...
const flowArray = [
    {
        flowCount: '0.0',
        flowName: '资金投放',
        flowNo: 'CapitalInvestment',
        visible: true
    },
    {
        flowCount: '0.0',
        flowName: '资金回笼',
        flowNo: 'FundCollection',
        visible: true
    },
    {
        flowCount: '0.0',
...
// yhf制度管理
const systemManagementFlowArray = [
    {
        flowCount: '0.0',
        flowName: '融租制度',
        flowNo: 'documentCheck',
        visible: false
    }
]
...
import JumpToH5Page from '../tempPageUsedtoJumpToH5Page/jumpToH5Page'
import FlowActions from '../../actions/FlowActions';
import CommonFunc from '../commonStyle/commonFunc';
import TaskDetailPage from '../todoTasks/todoTaskDetailPage';
import {RadioButton} from "../../components/radioBtnForm/SimpleRadioButton";
let subscription = null;
const LineIconCount = 4;
const HOME_PAGE_TODOLIST_COUNT = 30;
const showTypeIcons = false;//显示流程图标/流程待办

export default class WorkbenchForReport extends React.Component {
    constructor(props) {
        super(props);
        this._fetchPermission = this._fetchPermission.bind(this);
        ...
        this.state = {
            ...
            FixedAssetInventoryDataSource: Object.assign([], fixAssertsFlowArray),
            SystemManagementDataSource: Object.assign([], systemManagementFlowArray),
            refreshing: false,
            isShowingMore: true,
            //下面3个bool，负责2个报表+1个功能的权限部分的显示或者隐藏
            // 权限列表
            visibleFlag: []
        };
    }
    ...
    // yhf 和原文件workbench不同，这里不需要这个逻辑，所以删除
    _fetchData() {
    ...
    // 在切换到这个Tab页面的时候，就去获取权限，如果相应权限，就不显示这个部分
    _fetchPermission() {
        this.setState({refreshing: true});
        console.log("yhf userId for Permission Request " + AppStore.getUserName());
        LoginActions.getPermission({
            //        userid: this.state.username,// 'admin'
            userid: AppStore.getUserName(),// 'admin'
        }, (responseData) => {
            console.log("yhf data return in UI 111 :");
            console.log(responseData);
            // console.log("yhf ")
            let list = responseData.data;
            // var isPermittedToOpen = false;
            // let reportOrFunctionToShowlist =['资金投放','资金回笼','盘点'];
            const visibleFlag = [];
            for (var i = 0; i < list.length; i++) {
                var id = list[i].id;
                var user = list[i].useraccont;
                var name = list[i].power;
                visibleFlag.push(name)
            }
            ...
            this.lastFetchSuccessful = false;
            this.setState({refreshing: false});
            console.log("yhf  respone ERROR returned");
            // 可以显示获取权限失败
            Alert.alert(
                null,
                err,
    ...
    _getItemVisible = (item) => {
        ...
        const {visible, flowName} = item
        if (visible) {
            return visibleFlag.includes(flowName)
        } else {
            return true;
        }
    }
    ...
    _toJumpPageForH5(rowData) {
        ...
        // });
        // 转android native code
        // const flowName = rowData.flowName;
        // .then(response => response.json())
        let reportIndex = this._getReportIndex(rowData);
        let reportName = rowData.flowName;

        if (reportIndex === 2) {
            console.log("yhf  enter index 2 section");
            console.log("yhf userName = " + AppStore.getUserName());
            // Alert.alert('弹出二维码扫描',null,'确认');
        ...
        //        userid: this.state.username,// 'admin'
             ...
             console.log(responseData);
              //用到this需要注意指向，箭头函数
            this.setState({
                permittedReportlist:responseData.data
                })
                // console.log("yhf ")
                let list = responseData.data;
                var isPermittedToOpen = false;
                for(var i=0;i<list.length;i++){
                    ...
                    var name=list[i].power;
                    if(name == reportName){
                        isPermittedToOpen = true;
                        break;
                    }
                    console.log("yhf item id ="+id);
                    console.log("yhf item user ="+user);
                    console.log("yhf item name ="+name);
                }
                // 如果有权限，则打开，否则提示错误
                if(isPermittedToOpen){
                    console.log("yhf open it ...");
                    NativeModules.goToH5Page.startActivityFromJS("com.zjmileasing.app.H5PageActivity", reportIndex.toString(),reportName.toString());
                }else{
                    console.log("yhf can not open it !!!!");
                    Alert.alert(
                        null,
                        "没有权限打开",
                        [{text: '确定'}]
               ...
               console.log("yhf data return in UI 222:");
            //    console.log(responseData.data);
            //    console.log(responseData.data[0]);
            //    console.log("yhf length = "+responseData.data.length);
            //    console.log("yhf data return in UI 333:");
            //    for(var item in responseData.data){ //第一层循环取到各个list
            //    console.log("yhf item name ="+item['power']);
            //    }
        //     在这里出错
                ...
                console.log("yhf data return in UI 444 :");
    
                // console.log("yhf header: "+ header);
                // console.log("yhf body: "+ body);
            }, (err) => {
                console.log("yhf  respone ERROR returned");
                Alert.alert(
                    null,
        ...
        console.log("yhf  item name" + rowData.flowName + " index " + reportIndex);
        // let obj={
        //     id:1,
        //     name:rowData.flowName
        //   };
        //
        //        2是盘点的索引号，打开扫描界面 ，否则都是打开一个webview
        if (reportIndex == 2) {
            NativeModules
                .goToH5Page
                .startActivityFromJS("com.zjmileasing.app.activity.QRScannerActivity", reportIndex.toString(), reportName.toString(), AppStore.getUserName());
        } else {
            NativeModules
                .goToH5Page
                .startActivityFromJS("com.zjmileasing.app.activity.H5PageActivity", reportIndex.toString(), reportName.toString(), AppStore.getUserName());
        }
        ...
        // .startActivityFromJS("com.zjmileasing.app.JumpActivity", reportIndex.toString());
    ...
    _renderRow(rowData, isShowMore = false) {
        //  当没有权限的时候，不显示
        if (!this._getItemVisible(rowData)) {
            return null;
        }
        ...
        return (
            <TouchableOpacity activeOpacity={0.6} style={{width: width, paddingVertical: 10}} onPress={() => this._toJumpPageForH5(rowData)} >
    ...
    // yhf 在综合管理这一个tab页面， 综合报表下面的2个报表按钮 ＋ 固定资产下面的 ’盘点‘按钮，都以这边为逻辑下标，传给Android Activity
    // 在Android Activity里面根据下标 选择url，去加载不同的 h5页面
    _getReportIndex(rowData) {
        switch (rowData.flowNo) {
            case '综合报表':
            case 'ComprehensiveReport':

            case '资金投放':
            case 'CapitalInvestment':资金投放
                return 0;
            case '固定资产盘点':
            case 'FixedAssetInventory':
            case '资金回笼':
    ...
    // yhf
    /**因为渲染子界面类似，只不过子界面的内容文字不同，所以
     * type1 综合管理 -> 1.资金投放；2.资金回笼 下面的内容
     * type2 制度管理 -> 1.融租制度
     * type3 固定资产 -> 1.盘点
     * @param type
     * @returns {JSX.Element}
    ...
    _renderContentByType(type) {
        var strArr = [];
        if (type == '综合报表') {
            strArr = this.state.tipsDataSource || [];
        } else if (type == '制度管理') {
            strArr = this.state.SystemManagementDataSource || [];
        } else if (type == '固定资产') {
            strArr = this.state.FixedAssetInventoryDataSource || [];
        }

        // var strArr = this.state.FixedAssetInventoryDataSource || [];
        const length = strArr.length;
        if (length === 0) {
            return;
        }
    ...
    // yhf 综合管理 -> 固定资产 -> 盘点 下面的内容
    _renderContentForFixedAssets() {
        var strArr = this.state.FixedAssetInventoryDataSource || [];
        const length = strArr.length;
        if (length === 0) {
            return;
        ...
        return (
            <View style={{flex: 1, backgroundColor: 'white'}}>
                ...
                >
                    ...
                    {this._renderSubTitle('综合报表')}
                    {this._renderContent()}
                    {this._renderSubTitle('制度管理')}
                    {this._renderContentByType('制度管理')}
                    {this._renderSubTitle('固定资产')}
                    {this._renderContentForFixedAssets()}
    ...
    _renderSubTitle2(type) {
        const titleData = {
            process: {icon: require('../../image/ywlc.png'), text: '综合报表', more: false, marginTop: true, },
    ...
    render() {
        return (
            <NavigationBar
                ...
                title="物产融租业务系统"
                isEmptBottom={true}
                naviBarStyle={{backgroundColor: '#1BA0E5'}}  //此actionbar底色将作为android组件中toolbar的底色
                titleColor="white"
